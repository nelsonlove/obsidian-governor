...
The `rev` a read returns is advanced by **any** change to the note, not just MCP writes — another session's write, a human edit in the editor, a review tool restoring a note or stamping its frontmatter. That is the deliberate interplay with human review: a decision made *outside* MCP bumps the revision like any other write, so an agent still holding the old `rev` fails with `rev_conflict` instead of silently overwriting the human's action. Read-then-conditionally-write is the good-citizen pattern; unconditional writes remain available where last-write-wins is acceptable.
