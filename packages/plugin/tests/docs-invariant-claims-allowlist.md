...
- Two residuals are named, not papered over: a small, bounded false-positive surface (benign content that, under some reading, would form a fence asserting acceptance — which agents may not write anyway), and — for the backstop only — a single non-standard convention that would need *different* unmodeled outcomes at *different* escape positions to co-build one fence, which a global substitution cannot reach (covering it means treating every escape as a wildcard, whose false-positive cost is unbounded).
