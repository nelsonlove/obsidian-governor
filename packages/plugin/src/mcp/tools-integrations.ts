    ...
    server.registerTool(
      ...
      async ({ template_path, target_path }) => {
        try {
          ...
          // happens.
          //
          // `templateContentAcceptRefusal` fails closed on BOTH: (a) a
          // Templater expansion token (#137) — a `<% %>` tag can expand into an
          // acceptance assertion AND its fence from a template containing
          // neither, so a template the guard cannot inspect pre-render is
          // refused; and (b) a static accepted fence in the resolved bytes
