...
//      plugin's id is `vault-triage`, so `queue` is on the wire as
//      `vault_triage_queue`. The sanitizer is the host's `sanitizeOwnerId`,
...
//
// It deliberately does NOT reproduce the F3 pathless-tool block or the path
// allowlist: those are host code with host tests (`tests/external-tools.test.mjs`
// over there), and re-implementing them here would be a second copy that could
// drift into asserting a posture the host does not actually enforce.

const PLUGIN_ID = "vault-triage";
