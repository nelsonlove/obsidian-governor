...
Private-pack documentation exists to make the boundary explicit. It is not evidence that private code belongs in the submitted bundle. Beyond the exclusions above, two in-repo capabilities are **target-private in the first release** and must also be absent from the bundle: JD scaffolding and provenance regeneration ([Status and compatibility](../docs/status-and-compatibility.md)). Four further target-private capabilities, skills/policy compilation, triage mutations, cross-session coordination, and QuickAdd execution, are no longer part of this plugin: each has been extracted into its own satellite plugin in this monorepo (`packages/skills`, id `vault-skills`; `packages/triage`, id `vault-triage`; `packages/crosssession`, id `vault-crosssession`; `packages/quickadd-choices-compile`), built as a separate artifact from a separate manifest. They are absent from the submitted bundle structurally — the bundle is one plugin's `main.js` and `manifest.json` — rather than by an in-plugin exclusion a reviewer has to verify.
