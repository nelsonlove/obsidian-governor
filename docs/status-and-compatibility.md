...
- **Known durability gap (open): the standing chain has no backup coverage.** The chain that makes admission claims authoritative lives outside the vault and outside every backup; losing it reads as “nothing is admitted” rather than erroring (a critical health check now makes that state loud). Tracked as issue #337; unresolved, and the operator's ruling is pending. Do not read any durability promise in this suite as shipped while #337 is open.
