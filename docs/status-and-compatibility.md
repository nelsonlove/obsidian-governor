...
- **Known defect in 0.17.0–0.18.1 (fixed in 0.18.2): the file-explorer context-menu Accept was still offered after the cutover.** Choosing it refused correctly (`legacy_writer_disabled`) — no legacy write could land — but the item was offered-and-failing where the review pane already omitted it. 0.18.2 omits the menu item too.
