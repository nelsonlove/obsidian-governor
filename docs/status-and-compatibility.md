...
- **Known defect in 0.17.0–0.18.1 (fix on main, ships next release): the file-explorer context-menu Accept is still offered after the cutover.** Choosing it refuses correctly (`legacy_writer_disabled`) — no legacy write can land — but the item should be omitted rather than offered-and-failing, as it already is in the review pane. Until the fix ships, the menu item appears on eligible notes and can only fail.
