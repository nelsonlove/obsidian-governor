...
## Current release state — 0.17.0 (2026-08-23)
...
- **Highest completed gate: Gate 1** (work packages WP0–WP8), released as 0.17.0 and installed with the published asset byte-verified. Shipped with implementation evidence: the action registry and shared operation executor, observation/effect capture, canonical subjects, the Git-backed history store, replica-local sessions, proposals with recorded base/result snapshots, deterministic verification run at admission, gesture-gated individual and cohort admission (one gesture, one claim — enforced at runtime), a stock-git-readable standing chain advanced by compare-and-swap, and the legacy evidence import with a human-confirmed cutover capability.
- **Everything above ships INERT.** Observation capture, the history store, and the cutover flag are off/absent by default and were verified absent from the live install's settings. Nothing in Gate 1 acts until the operator enables it.
- **The authority cutover has NOT run.** The [legacy acceptance system](acceptance-model.md) is still the live standing authority. The cutover is a capability, not a state: it happens only through the operator's confirmed click (see the [cutover runbook](../packages/plugin/docs/wp8-cutover-runbook.md)), and until then the legacy writers remain active.
- **Known durability gap: the standing chain has no backup coverage today.** The chain that makes admission claims authoritative lives outside the vault; losing it reads as “nothing is admitted” rather than erroring (a critical health check now makes that state loud). Tracked as issue #337; **ruled 2026-08-23** (the history store comes under backup coverage, excluding observation replay payloads — an exclusion on size, not privacy grounds) but **not yet implemented** — the change belongs to the vault backup job, not to this plugin, so no Governor release closes it. The chain remains uncovered until that lands; do not read any durability promise in this suite as shipped meanwhile.
- **Known defect in 0.15.0–0.17.0 (fixed after 0.17.0): gesture-gated controls are silently inert in popout windows.** The gesture gate's realm-bound `instanceof` check refused genuinely trusted clicks delivered in a second window (settings popouts, popped-out review panes) with no feedback — a false negative; no forged input was admitted. Workaround on affected builds: use the main window. The fix is a realm-safe platform-Event brand check, and blocked clicks now surface a Notice.
- **Known gap in 0.17.0 (fix on main, ships next release): the cutover marker travels with the vault while the standing chain is machine-local.** A vault restore or copy carries `cutover.json` (“cut over”) without the chain, and admission answers on such a machine read as “nothing admitted” — indistinguishable from truth at exactly the disaster-recovery moment. The fix binds the marker to a store identity that lives inside the machine-local history dir (it rides the chain backup, never the vault): an unmatched machine reports “cut over elsewhere; chain absent here” and admission refuses there; a pre-binding marker is resolved by ONE gesture-gated bind on the machine holding the real chain — never automatically. Until the fix ships AND the bind is taken: treat the machine that ran the cutover as the only authoritative one, including after any restore.
...
| Skills/policy compilation, cross-session coordination, JD scaffolding, triage mutations, QuickAdd execution, provenance regeneration | Target private in the first release | Absent from the Community bundle; promotion requires a separate reviewed manifest change |
