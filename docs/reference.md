...
- **`obsidian_renumber_address`** — `{path, to, scheme?, dry_run, on_occupied, displace_to?}` → moves the note to a specific target address `to`; if occupied, `on_occupied` (`"fail"` default / `"auto"` / `"manual"`) decides whether to refuse or displace the occupant (to its own scope's next free slot, or to `displace_to`) — the occupant's move always runs before the source note's own.
