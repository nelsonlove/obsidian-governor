...
7. **[a decision S2 cannot start without]** Settle session ownership: either the provider owns sessions and the seam gains a connection-lifecycle notification, or the host keeps minting and `SessionV1` becomes a published contract. S2 cannot be built without the answer.
