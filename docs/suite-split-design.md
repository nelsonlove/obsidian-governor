...
**The nine conditions S2 must satisfy:**

1. Registered hooks unreachable from `app` (module-private WeakMaps), pinned by a runtime transitive walk plus a source tripwire; `main.ts`'s `externalRegistry` fixed in the same motion.
2. No permission-shaped hook (rule 6), pinned as a source scan over the seam's declared return types; if any boolean survives, deny-wins unanimity with two-disagreeing-registrants tests.
3. Disposer-only revocation, pinned: a caller holding the api object cannot revoke a hook it did not register.
4. Defensive-copy `WriteFacts`, pinned three ways (byte mutation, actor mutation, per-hook independence).
5. Placement and hang budgets: vetoes and the gate run INSIDE the kernel's queued closure where `sessionLive` runs today, so `WRITE_TIMEOUT_MS` bounds them (a hanging veto is abandoned and the write refused — no new timeout machinery needed); observers are NOT awaited on the caller's result path (a hanging observer must not cost a write that already landed).
6. **Provider absence must be distinguishable from provider-never-installed** (the review's finding 3, and the biggest doctrinal cost of the split): post-split, `obsidian_plugin_toggle('governor', false)` is a one-call, default-settings way for an AGENT to remove governance — today impossible only because host and provider are one artifact. The host persists a latch on first provider registration; thereafter the plugin-toggle/uninstall tools refuse the latched id, and a latched-but-hookless host refuses mutating tools until a human clears it.
7. Settle session ownership: either the provider owns sessions and the seam gains a connection-lifecycle notification, or the host keeps minting and `SessionV1` becomes a published contract. S2 cannot be built without the answer.
